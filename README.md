...
**Batch 6 local gate status.** 威胁模型、精选负向安全门禁和固定合成性能门禁的实现基线为 `f42a56d8cc20632e12ea6e21e8f64ffbf7be6cd8`；统一的本地证据基线为 `62de8ae81c27e146e3a2e8b831d85c41ac9f71d4`。该增量未修改 `src/agentguardian`，OpenAI Provider 仍只做本地检测与人工指引，不默认调用 API。独立只读复审、当前精确 SHA 的 GitHub CI、fresh-runner provenance、可信签名、原生安装/卸载和许可复核仍待完成。Release-candidate decision: `NO-GO`. Windows MVP remains incomplete. Production safety is not established.
