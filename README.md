...
- A network-neutral enterprise service boundary now enforces tenant-scoped role checks, hashed admin tokens, signed-policy provisioning, device revocation, and bounded audit export. It opens no socket; TLS, remote enrollment, key rotation, rate limiting, deployment authentication, and independent service review remain release gates.
