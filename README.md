...
**工作流与报告硬化 Batch 4 当前状态。** Task 1-8 已在本地实现，Task 9 完整本地门禁已重新通过；Task 10 的独立复审和最终 SHA 远程验收尚未完成。每次扫描都需要与当前范围绑定的明确同意；范围变化会撤销同意并清除旧结果，扫描启动前会再次校验并消费本次同意。范围预览不遍历目录，只显示短名称、支持的选择器和固定扫描上限。
...
残余限制包括同一用户控制、路径竞态、主机时钟、路径别名、聚合碰撞，以及静态自审计不覆盖依赖和二进制。Task 9 本地门禁在 Python 3.14 和隔离的 Python 3.12 环境中均为 `1174 passed, 8 skipped, 0 failed`。8 项真实 symlink 用例因本机 symlink 创建权限不足而跳过，junction 已测试；这些 skip 不算对应 symlink 场景通过。当前 Batch 4 GitHub CI 尚未重新验证。Batches 5-6 仍待完成，Windows MVP 尚未完成，未形成生产安全结论。
