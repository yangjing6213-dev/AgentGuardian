...
- Offline enterprise policy enforcement is implemented: canonical policy parsing, role/capability allowlists, expiry, operator-provisioned SHA-256 pinning, and high-sensitivity confirmation. The desktop now exposes a local-only control-plane page for tenant/device/role registration, policy import, device revocation, and bounded operational summaries. It is not a signed policy service, remote administrator console, or enterprise SaaS control plane.
