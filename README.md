...
- The native Windows MCP path rechecks the adapter SHA-256, requires a locally trusted embedded Authenticode signature, and requires an explicit exact-match X.500 publisher-subject allowlist plus exact signer-certificate SHA-256 pin before launch. It now accepts only the fixed adapter inside the OS-resolved `yangjing6213dev.AgentGuardian` Store/line-of-business package under the Program Files `WindowsApps` repository; `Install` and `Effective` roots must match, while Mutable and all External roots must be absent. The complete path must also deny the current token delete, child-delete, DACL/owner mutation, and file-add rights. The validated executable is held through process creation and WinTrust receives that same handle. Portable or user-writable adapters remain default-deny. An organization-signed adapter artifact and clean-machine acceptance remain release gates.
- Current local revalidation for implementation/test revision `1da903465f463d1421e7af2b20971da3d8c149bd` (runtime hardening SHA `6ccb5232f6eb3955890f89f7a1000df338db8e8a`): full suite `1563 passed, 11 skipped`; Windows MVP security gate `47 passed, 1 skipped`; `python -m compileall -q src scripts tests` exit 0; and `git diff --check` clean. Exact-SHA push CI `31929152189`, push Windows `31929152263`, Draft PR CI `31929154481`, and Draft PR Windows `31929154483` all succeeded. Pre-repair remote HEAD `1af14022acc47a053d6393bea05a171a913ca84a` and documentation HEAD `ac796dd379b785d3c52aa6a7e2dde52d078d7e78` remain failed historical evidence for the Windows rename-variance test and are not reported as passing.
...
- Trusted builds require all four external MCP adapter inputs: path, exact SHA-256, exact X.500 publisher subject, and exact DER certificate SHA-256. Under a source-file lock they verify the actual adapter bytes, trusted Authenticode, publisher, and certificate pin; stage only `adapters/AgentGuardianMcpAdapter.exe` and `MCP-ADAPTER.json`; then rehash and hold the staged adapter without write/delete sharing through payload evidence and ZIP generation. Adapter inputs remain forbidden in unsigned builds.
- Runtime launch, trusted staging, packaged acceptance, and the final release-evidence gate now share one streaming SHA-256 helper with an exact 64 MiB adapter limit. Exact-limit inputs are accepted; `64 MiB + 1 byte`, including a real sparse-file case, fails closed before launch, copy, or evidence creation. Manifest and ZIP processing can still allocate up to the bounded 64 MiB artifact size.
- The manual trusted-signature workflow requires four non-secret repository variables. Its standard-library downloader accepts only a direct HTTPS URL without credentials, query, fragment, or redirects; streams at most 64 MiB into a random private runner directory; binds the Win32 file handle to its resolved final path; and deletes failed downloads through that handle. Organization PFX/password values are step-scoped; imported certificates/private keys and the PFX are removed and residue-checked fail-closed. The job has a 30-minute outer timeout.
- Trusted MSIX acceptance now binds the exact source SHA, requires fresh user state and a completed same-identity upgrade, validates the installed adapter's strict package path and reparse-free components, and executes it under AppContainer plus a Job Object. Its bounded evidence binds the actual adapter bytes, X.500 publisher subject, DER certificate SHA-256, completed native sandbox metadata, and `raw_response_retained=false`; uninstall cleanup requeries and retries before the final source/signature/license/MCP gate.
...
- Task 1, Task 2, trusted-artifact acquisition hardening, bounded adapter hashing, and the packaged-path identity repair passed independent specification or quality/security review with no remaining Critical or Important issues. The path review required Store/line-of-business origin, the Program Files `WindowsApps` install repository, exact `Install == Effective`, and explicit rejection of Mutable and External package paths before it passed. This review result and the local tests do not replace external-material or runtime acceptance.
- `windows-mvp-signed.yml` has not been dispatched or passed with a real organization adapter/certificate. Required repository variables, signing material, approved `windows-license-review.json`, real sanitized-sample human signoff, and independent clean-machine install/upgrade/run/uninstall evidence remain pending. Remote enterprise console, device enrollment, and distribution remain unimplemented.
- Residual work remains: package/path checks and final `CreateProcessW` are not one atomic kernel operation, so the current claim is limited to a standard-user process and a Windows-managed immutable package repository, not administrator/SYSTEM or a compromised deployment service. Multi-signature signer-index binding, evidence-output parent-path TOCTOU, and synchronous AppX operations bounded only by the workflow's outer timeout also remain open. No production safety, support for highly sensitive real-world data, or legal approval is claimed.
