...
- Current local revalidation for clean code-bearing HEAD `6ccb5232f6eb3955890f89f7a1000df338db8e8a`: full suite `1563 passed, 11 skipped`; Windows MVP security gate `47 passed, 1 skipped`; `python -m compileall -q src scripts tests` exit 0; and `git diff --check` clean. Current normal GitHub CI for this local SHA has not yet been revalidated. Pre-repair remote HEAD `1af14022acc47a053d6393bea05a171a913ca84a` failed Windows run `31907590797` because `FileRenameInfoEx` POSIX replacement succeeded despite the held file; predecessor `f4b3d5c5bfd9bd4f8f6733ac9dad491c7d2bb47e` remains the latest exact SHA with all four normal checks green.
