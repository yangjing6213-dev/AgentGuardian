...
- The native Windows MCP path rechecks the adapter SHA-256, requires a locally trusted embedded Authenticode signature, and requires an explicit exact-match X.500 publisher-subject allowlist plus exact signer-certificate SHA-256 pin before launch. The validated executable is held open without write/delete sharing through process creation. Empty or non-matching allowlists are denied. An organization-signed adapter artifact and clean-machine acceptance remain release gates.
- Current local revalidation for clean code-bearing HEAD `75fd3d9f0f8c8921fecac13f6eec534c5094140f`: full suite `1529 passed, 11 skipped`; Windows MVP security gate `47 passed, 1 skipped`; `python -m compileall src scripts tests` exit 0; and `git diff --check` clean. Current normal GitHub CI for this local SHA has not been revalidated; predecessor `aabd6e828ea208d4e80951cb02ea5ead12750797` passed exact-SHA push and Draft PR CI plus both Windows package checks.
...
- Task 1, Task 2, and the trusted-artifact acquisition hardening passed independent specification or quality/security review with no remaining Critical or Important issues. This review result and the local tests do not replace external-material or runtime acceptance.
...
- Residual Minor/defense-in-depth work remains: runtime WinTrust verification is still path-based inside the locked launch window rather than explicitly bound to that handle; multi-signature signer-index binding; evidence-output parent-path TOCTOU; streaming/size bounds in acceptance and sandbox hashing; and synchronous AppX operations bounded only by the workflow's outer timeout. No production safety, high-sensitive real-data readiness, or legal approval is claimed.
