...
- [x] CI 使用短期自签名证书只做安装烟测；该证书不得进入发布物或被当作可信签名。
- [ ] 在 Windows Runner 执行真实 MakeAppx、签名和安装卸载烟测。
...
以下任一项缺失，发布状态保持 `NO-GO`：当前精确 SHA 远程 CI、可信签名、SBOM/许可证复核、独立干净机安装卸载、真实数据验收、关键安全复审或动态能力隔离证据。开发自签名包只允许 CI smoke，不允许公开交付。
