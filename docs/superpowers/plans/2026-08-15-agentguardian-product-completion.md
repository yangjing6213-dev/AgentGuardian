...
- [x] 增加显式开发/测试回环适配器：只允许字面 `127.0.0.1`、不默认启动、串行化 SQLite 请求并以固定错误响应隔离内部异常；这不是 TLS 远程服务。
...
- Repository HEAD `e5c279a` adds the dynamic MCP publisher-attestation contract: explicit policy allowlists, exact X.500 subject matching through local Windows Crypt32 APIs, and separate signature/allowlist denial reasons. Local verification is `1436 passed, 11 skipped` plus the selected security gate `47 passed, 1 skipped`; push CI `31890969434`, push Windows `31890969501`, Draft PR CI `31890971629`, and Draft PR Windows `31890971646` all succeeded. Trusted organization signing, packaged-adapter acceptance, clean-machine installation/uninstall, sanitized-sample signoff, license approval, and independent final review remain open.
- Repository HEAD `e8013dc` extends the dynamic MCP publisher gate with exact signer-certificate DER SHA-256 pins, holds the validated executable open without write/delete sharing through process creation, and fails closed on Crypt32 cleanup errors. Local verification is `1438 passed, 11 skipped` plus the selected security gate `47 passed, 1 skipped`; push CI `31892391687`, push Windows `31892391681`, Draft PR CI `31892393406`, and Draft PR Windows `31892393403` all succeeded. Trusted organization signing, packaged-adapter acceptance, clean-machine installation/uninstall, sanitized-sample signoff, license approval, and independent final review remain open; release status remains `NO-GO`.

## Task 3 Documentation Synchronization (2026-08-16)

- Clean code-bearing HEAD `3febfd57b6841181597bd5476e176710e81a011f` implements Task 1 packaged MCP adapter acceptance and final trusted release evidence binding. Evidence binds the exact source SHA, actual fixed adapter bytes, exact X.500 publisher subject, exact DER certificate SHA-256, bounded completed native sandbox metadata, and `raw_response_retained=false`.
- Task 2 requires all four external adapter inputs for trusted builds, verifies exact hash plus trusted Authenticode plus exact publisher/certificate pin under a source-file lock, stages only `adapters/AgentGuardianMcpAdapter.exe` and `MCP-ADAPTER.json` before payload manifests/checksums/ZIP, and rejects adapter inputs in unsigned builds.
- The manual signed workflow requires four non-secret repository variables, an absolute HTTPS fixed-path download, and a pre-build hash check. Organization PFX/password material is step-scoped; imported certificate/private-key and PFX cleanup are residue-checked fail-closed. The job has a 30-minute outer timeout.
- Trusted MSIX acceptance requires source SHA, fresh user state, completed same-identity upgrade, strict installed path/reparse checks, installed adapter execution under AppContainer plus a Job Object, bounded evidence, bounded requery/retry uninstall cleanup, and the final source/signature/license/MCP evidence gate.
- Task 1 and Task 2 passed independent specification and quality/security review with no remaining Critical or Important issues. Fresh local verification recorded `1510 passed, 11 skipped`, Windows MVP security gate `47 passed, 1 skipped`, compileall exit 0, and clean `git diff --check`. Earlier independent `1510 passed, 11 skipped` and focused Task 2 `117 passed, 1 skipped` results remain supporting context only.
- OpenAI Provider remains local detection/manual guidance first and makes no default API call.
- Implementation exists, but `windows-mvp-signed.yml` has not been dispatched or passed with a real organization adapter/certificate. Required repository variables, signing material, approved `windows-license-review.json`, real sanitized-sample human signoff, and independent clean-machine install/upgrade/run/uninstall evidence remain pending. Implementation/test revision `1da903465f463d1421e7af2b20971da3d8c149bd` passed all four normal GitHub checks; historical CI remains historical.
- Previous code-bearing HEAD `db56b30` bound runtime WinTrust verification to the same executable handle held without write/delete sharing through launch. Real Windows tests covered handle propagation and parent-directory replacement denial while locked; focused independent re-review ended with no Critical or Important findings. The trusted-adapter downloader and staged-adapter locking controls from `75fd3d9f0f8c8921fecac13f6eec534c5094140f` remain in place. Historical documentation follow-up `d22049dfdcfe177a760d3a3626d67a4fd020a825` passed exact-SHA push and Draft PR CI plus both Windows package checks.
- Code-bearing HEAD `6ccb5232f6eb3955890f89f7a1000df338db8e8a` retains the streaming 64 MiB MCP adapter limit and repairs the path replacement assumption invalidated by remote HEAD `1af14022acc47a053d6393bea05a171a913ca84a` Windows run `31907590797`. Runtime now requires the OS-resolved `yangjing6213dev.AgentGuardian` Store/line-of-business package under Program Files `WindowsApps`, `Install == Effective`, no Mutable or External roots, and no current-token path mutation rights. Package full name is cross-bound across MSIX smoke, MCP acceptance, and the final release gate. Final independent re-review found no Critical or Important issues. Implementation/test revision `1da903465f463d1421e7af2b20971da3d8c149bd` preserves that runtime and records fresh local verification of `1563 passed, 11 skipped`, security gate `47 passed, 1 skipped`, compileall exit 0, and clean `git diff --check`; its exact-SHA push and Draft PR CI plus both Windows package checks all succeeded.
- Residual work includes the non-atomic package/path-check and final-`CreateProcessW` boundary; the claim is limited to a standard user and a Windows-managed immutable package repository, excluding administrator/SYSTEM and a compromised deployment service. Multi-signature signer-index binding, evidence-output parent-path TOCTOU, and synchronous AppX operations bounded only by the outer workflow timeout remain open. Remote enterprise console/device enrollment/distribution remains unimplemented.
- Release decision remains `NO-GO`. No production safety, high-sensitive real-data readiness, or legal approval is claimed.
