...
| 2 | [Protected local evidence state](2026-08-02-agentguardian-protected-evidence-state.md) | Persist only rule IDs, masked summaries, scan metadata, and HMAC references under Windows DPAPI; never persist raw matches or scan keys; corrupted state fails closed. |
