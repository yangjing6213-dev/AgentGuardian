...
| --- | --- | --- | --- | --- |
| AG-T13 | A dynamic MCP adapter starts with an unreviewed executable, mutable bytes, shell expansion, unbounded output, or no process/network boundary. | `mcp_sandbox.py` requires an absolute regular executable, expected SHA-256, fixed argv, explicit confirmation, bounded request/output/runtime, temporary working directory, and an internal native-isolation attestation. `windows_job_object.py` locally proves the Windows process-tree boundary; without a complete provider it still refuses to start. | partial-local | The current portable/MSIX runtime has no proven network-deny provider, signed adapter, clean-machine install/uninstall, or crash-recovery evidence; synthetic attestation is unit-test-only. |
