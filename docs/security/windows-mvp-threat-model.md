...
| --- | --- | --- | --- | --- |
| AG-T13 | An MCP configuration declares a dangerous capability combination, contains malformed data, or leaks server names and source paths into evidence. | `detect_mcp_config` parses the selected configuration as bounded static data, requires shell, filesystem-write, and network capabilities on the same server before emitting `MCP_DANGEROUS_COMBINATION`, and retains only a fixed masked summary plus local HMAC references. Personal v1 has no MCP download, load, launch, brokering, isolation, signing, packaging, or execution path. `tests/test_detectors.py::test_mcp_combination_requires_all_capabilities_on_same_server` | verified-local | Static configuration analysis cannot prove actual server behavior, runtime permissions, or configuration completeness. Unselected and unsupported configuration sources remain outside coverage. |
| AG-T14 | Removed enterprise, high-sensitivity, or dynamic-extension capabilities are mistaken for current Personal functionality or reintroduced into source, payload, or active documentation. | The personal release profile rejects retired source and payload names plus exact active-document implementation claims. Personal v1 retains static MCP configuration detection only. | verified-local | This is source, layout, and documentation evidence; it does not inspect compiled binary semantics or prove runtime behavior. |
