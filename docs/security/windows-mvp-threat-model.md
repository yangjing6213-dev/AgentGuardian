...
| --- | --- | --- | --- | --- |
| AG-T13 | A dynamic MCP adapter starts with an unreviewed executable, mutable bytes, shell expansion, oversized artifact, unbounded output, or no process/network boundary. | `mcp_sandbox.py` requires an absolute regular executable, expected SHA-256, fixed argv, explicit confirmation, bounded request/output/runtime, temporary working directory, and native isolation. Windows uses AppContainer plus Job Object, requires locally trusted embedded Authenticode, exact X.500 subject and exact signer-certificate DER SHA-256, holds the executable without write/delete sharing through launch, and passes that native handle to WinTrust. Runtime, staging, packaged acceptance, and final release verification share a streaming 64 MiB adapter hash limit. Real Windows tests confirm parent-directory and `FileRenameInfoEx` POSIX replacement are denied while the file is held. Native provider failures remain default-deny. | partial-local | The allowlist and size controls are implemented, but a real organization adapter identity is not configured or accepted. The reparse-check/CreateFile boundary is not atomic, multi-signature signer-index binding remains open, and crash/restart acceptance, clean-machine install/uninstall, and independent signed-adapter evidence are pending. |
