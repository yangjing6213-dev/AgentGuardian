...
| --- | --- | --- | --- | --- |
| AG-T13 | A dynamic MCP adapter starts with an unreviewed executable, mutable bytes, shell expansion, oversized artifact, unbounded output, or no process/network boundary. | `mcp_sandbox.py` requires an absolute regular executable, expected SHA-256, fixed argv, explicit confirmation, bounded request/output/runtime, temporary working directory, and native isolation. Windows uses AppContainer plus Job Object, locally trusted embedded Authenticode, exact X.500 subject and signer-certificate DER SHA-256, and handle-bound WinTrust. It additionally requires the fixed adapter under the OS-resolved `yangjing6213dev.AgentGuardian` Store/line-of-business package in Program Files `WindowsApps`, exact `Install == Effective`, no Mutable or External package path, and no current-token path mutation rights. Runtime, staging, packaged acceptance, and final release verification share a streaming 64 MiB adapter hash limit. The real POSIX probe permits both documented Windows outcomes but proves a user-writable path is always rejected. Native provider failures remain default-deny. | partial-local | GitHub Windows run `31907590797` proved that a held file alone does not prevent POSIX replacement across environments. The repaired boundary is limited to a standard user and a Windows-managed immutable package repository; administrator/SYSTEM or deployment-service compromise is out of scope. A real organization adapter identity is not configured or accepted, multi-signature signer-index binding remains open, and crash/restart acceptance, clean-machine install/uninstall, and independent signed-adapter evidence are pending. |
| AG-T14 | A locally modified enterprise policy grants capabilities to the wrong device, role, or version. | `enterprise_policy.py` rejects duplicate/unknown fields, validates tenant/device binding, expiry, role allowlists, high-sensitivity confirmation, SHA-256 pin mismatch, and version rollback. Optional `enterprise_signing.py` verifies an Ed25519 envelope before signed provisioning; the local control plane provides device registration and revocation. | partial-local | The signed envelope is opt-in and has no remote key registry, device service, tenant isolation service, policy distribution, or administrator authentication/console. |
