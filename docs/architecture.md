...
静态自审计先严格读取随包分发的 `source_policy.json` schema 1 清单，并要求包内 `.py` 模块集合与清单完全相等；每个已复核模块必须匹配基于 `ast.dump(..., annotate_fields=True, include_attributes=False)` 的 canonical AST SHA-256。任何模块增加、删除、无法解析或 AST 变化都会以 `SOURCE_POLICY_VIOLATION` 失败关闭并令 `local_only` 为 false，已复核包不会再由启发式解释。有限启发式仅对清单外的合成未知模块运行，用于识别代表性的网络导入、动态执行和用户数据写入能力；它不是 Python 表达式解释器，也不构成语义证明。
